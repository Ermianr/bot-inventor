...
The first is that `apps/web` had no `setupFiles`, so `globalThis.IS_REACT_ACT_ENVIRONMENT` was never set. (The flag is still set once for the whole app; where it is set moved with the test runner, and [ADR 0017](./0017-the-test-runner-is-bun.md) is that change. The decision here holds — only its mechanism moved.) Every `act()` in every component test printed "The current testing environment is not configured to support act(...)" and then did not do the thing its name says: React leaves the work the scope caused to whatever the scheduler gets round to rather than flushing it before handing back. That is now set once, for every test in the app, and the warning is gone.
...
So we decided that **a test interacts with a control only once React has finished with it**. `settled()` — an empty `await act()` — flushes what React has pending; it is not a delay. `press()` is `settled()` and a click together, and it is how a test presses anything: the rule is the ordinary way of doing the thing rather than something to remember beside it, because a test that forgets it does not fail, it passes on a quiet machine.
...
- **`press()` is how a test presses something.** The tests written before it stay as they are until they are touched: rewriting them all would be a large diff to buy nothing, since a control that was already on the screen is one `press()` does nothing extra for.
- **`settled()` stays for what is not a press.** Typing into a field that has just appeared, or a test that deliberately looks at the screen before the store has answered and has to own the answer it left behind.
